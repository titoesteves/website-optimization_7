...
var resizePizzas = function(size) { 
  ...
  // Removed determineDx and sizeSwitcher functions
  // Simplified conversion from percentage to instead whole number. e.g. 
  ...
  function changePizzaSizes(size) {
    ...
    //created var to not repeat and follow dry convention
...
// Moved variable outside of for loop since it does not change
...
function updatePositions() {
  ...
  //Created a separate for loop to improve performance since there are only five possible results
  var phases = [];
  for (var i = 0; i < 5; i++){
    phases[i] = Math.sin((document.body.scrollTop / 1250) + i );
  }

  // storing in variable, so it does not have to be checked each time in loop 
